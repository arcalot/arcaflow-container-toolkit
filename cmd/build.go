...
func BuildCmdMain(build_img bool, push_img bool, cec ce_client.ContainerEngineClient, conf config, abspath string,
	...
	if err := BuildImage(build_img, all_checks, cec, abspath, conf.Image_Name, conf.Image_Tag, conf.Quay_Exp,
		logger); err != nil {
		return false, err
	}
...
func BuildImage(build_img bool, all_checks bool, cec ce_client.ContainerEngineClient, abspath string, image_name string,
	image_tag string, quay_exp string, logger log.Logger) error {
	...
	if all_checks && build_img {
		...
		if err := cec.Build(abspath, image_name, []string{image_tag}, quay_exp); err != nil {
			return err
		}
	}
	return nil
...
func getConfig(logger log.Logger) (config, error) {
	...
	conf := config{
		...
		Quay_Exp:         viper.GetString("quay_exp"),
